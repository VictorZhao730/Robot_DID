...
function verifyAnchorIssuanceTiming(
  ...
  const safeMaxDelay =
    Number.isFinite(maxDelay) && maxDelay >= 0 ? maxDelay : DEFAULT_MAX_PUBLISH_DELAY_SECONDS;
...
function getMaxPublishDelaySeconds() {
  const value = Number(document.getElementById("maxPublishDelaySeconds")?.value);
